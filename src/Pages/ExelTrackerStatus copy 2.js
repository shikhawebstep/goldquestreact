        ...
        if (newExpandedRows.includes(index) && services) {
            ...
            Promise.all(
                ...
                return Promise.all(annexureFetches).then(annexureStatusArr => {
                    setDBHeadingsStatus(annexureStatusArr);
                    const serviceHeadingsTR = document.getElementById(`service-table-headings-${id}`);
                    const serviceStatusTR = document.getElementById(`service-table-status-${id}`);
                    
                    console.log(`Appending to headings row: ${serviceHeadingsTR}`);
                    console.log(`Appending to status row: ${serviceStatusTR}`);
                    
                    dbHeadingsStatus.forEach((value, index) => {
                        console.log(`Processing entry ${index}:`, value);
                    
                        // Create and append heading <td>
                        const headingTd = document.createElement('td');
                        headingTd.className = `service-th-${id}`; // Dynamic class based on id
                        headingTd.textContent = value && value.db_table ? value.db_table : 'N/A';
                        
                        console.log(`Heading TD created with text: "${headingTd.textContent}"`);
                        serviceHeadingsTR.appendChild(headingTd);
                    
                        // Create and append status <td>
                        const statusTd = document.createElement('td');
                        statusTd.className = `service-td-${id}`; // Consider using a different class name for status
                        statusTd.textContent = value && value.status ? value.status : 'N/A';
                    
                        console.log(`Status TD created with text: "${statusTd.textContent}"`);
                        serviceStatusTR.appendChild(statusTd);
                    });
                    
                    // Final log to show the updated rows
                    console.log('Final headings row:', serviceHeadingsTR.innerHTML);
                    console.log('Final status row:', serviceStatusTR.innerHTML);
                                            ...
                                            <td colSpan="9" className="p-0">
                                                <div className='collapseMenu overflow-auto w-full max-w-[1500px]'>
                                                    <table className="min-w-full max-w-full bg-gray-100">
                                                        ...
                                                        <tbody>
                                                            <tr className={`service-table-status-${item.id}`}>
